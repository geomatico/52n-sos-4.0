...
public class DateTimeHelper {
	 ...
	 *             IF an error occurs.
	 */
	public static DateTime parseIsoString2DateTime(String timeString)
			throws OwsExceptionReport {
		...
		try {
			if (timeString.contains("+") || Pattern.matches("-\\d", timeString)
					|| timeString.contains("Z") || timeString.contains("z")) {
				return ISODateTimeFormat.dateOptionalTimeParser()
						.withOffsetParsed().parseDateTime(timeString);
			} else {
				return ISODateTimeFormat.dateOptionalTimeParser()
						.withZone(DateTimeZone.UTC).parseDateTime(timeString);
			}
		} catch (IllegalArgumentException iae) {
			String exceptionText = "Error while parse time String to DateTime!";
			LOGGER.error("Error while parse time String to DateTime!", iae);
			OwsExceptionReport owse = new OwsExceptionReport(iae);
			owse.addCodedException(OwsExceptionCode.InvalidParameterValue, null,
					exceptionText);
			throw owse;
		} catch (UnsupportedOperationException uoe) {
			String exceptionText = "Error while parse time String to DateTime!";
			LOGGER.error("Error while parse time String to DateTime!", uoe);
			OwsExceptionReport owse = new OwsExceptionReport(uoe);
			owse.addCodedException(OwsExceptionCode.InvalidParameterValue, null,
					exceptionText);
			throw owse;
		}
	 ...
	 *             If an error occurs.
	 */
	public static String formatDateTime2ResponseString(DateTime dateTime)
			throws OwsExceptionReport {
	 ...
	 *             If an error occurs.
	 */
	public static String formatDateTime2FormattedString(DateTime dateTime,
			String dateFormat) throws OwsExceptionReport {
		try {
			if (dateFormat == null || dateFormat.equals("")) {
				return formatDateTime2IsoString(dateTime);
			} else {
				if (dateTime == null) {
					return new DateTime(0000, 01, 01, 00, 00, 00, 000,
							DateTimeZone.UTC).toString(DateTimeFormat
							.forPattern(dateFormat));
				}
				return dateTime.toString(DateTimeFormat.forPattern(dateFormat))
						.replace("Z", "+00:00");
			}
		} catch (IllegalArgumentException iae) {
			String exceptionText = "Error while parse time String to DateTime!";
	 ...
	 */
	public static DateTime setDateTime2EndOfDay4RequestedEndPosition(
			DateTime dateTime, int isoTimeLength) {

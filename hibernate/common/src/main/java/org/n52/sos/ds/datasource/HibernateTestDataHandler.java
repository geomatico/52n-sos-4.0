...
public class HibernateTestDataHandler {
	private static final String[] offeringIds = new String[] {
	...
	private static final String[] procedureIds = new String[] {
			"http://www.52north.org/test/procedure/1",
			"http://www.52north.org/test/procedure/2",
			"http://www.52north.org/test/procedure/3",
			"http://www.52north.org/test/procedure/4",
			"http://www.52north.org/test/procedure/5",
			"http://www.52north.org/test/procedure/6",
			"http://www.52north.org/test/procedure/7",
			"http://www.52north.org/test/procedure/8" };

	private static final String[] observablePropertyIds = new String[] {
			...
			"http://www.52north.org/test/observableProperty/4",
			"http://www.52north.org/test/observableProperty/5",
			"http://www.52north.org/test/observableProperty/6",
			"http://www.52north.org/test/observableProperty/7",
			"http://www.52north.org/test/observableProperty/8" };

	private static final String[] featureIds = new String[] {
	...
	private static final String[] unitIds = new String[] { "test_unit_1",
			"test_unit_2", "test_unit_3", "test_unit_4", "test_unit_5",
			"test_unit_6", "test_unit_7", "test_unit_8" };
	...
	public static boolean hasTestData() {
		Set<String> procedures = Configurator.getInstance().getCache()
				.getProcedures();
		for (String procedureId : procedures) {
			if (procedureId.equals(procedureIds[0])) {
				return true;
			}
		}
	...
	private void createOfferings() {
		String[] observationTypeIds = new String[] {
		...
		for (int i = 0; i < offeringIds.length; i++) {
			...
			offering.setIdentifier(offeringIds[i]);
			offering.setName(offeringIds[i]);
			offering.getFeatureOfInterestTypes().add(
					getFeatureOfInterestType(featureTypeId));
			offering.getObservationTypes().add(
					getObservationType(observationTypeIds[i]));
	...
	private void createFeatures() {
		...
		Point[] points = new Point[] {
				gf.createPoint(new Coordinate(7.727958, 51.883906)),
				gf.createPoint(new Coordinate(-117.1957110000000, 34.056517)),
				gf.createPoint(new Coordinate(6.1320144042060925,
						50.78570661296184)),
		...
		String[] names = new String[] { "con terra", "ESRI", "Kisters", "IfGI",
				"TU-Dresden", "Hochschule Bochum", "ITC", "DLZ-IT" };

		for (int i = 0; i < featureIds.length; i++) {
			TFeatureOfInterest feature = new TFeatureOfInterest();

			feature.setNames(names[i]);
			feature.setGeom(points[i]);
			feature.setIdentifier(featureIds[i]);
			feature.setFeatureOfInterestType(getFeatureOfInterestType(type));

			feature.setDescriptionXml("<sams:SFSpatialSamplingFeature "
	...
	private void createObservableProperties() {
		for (String id : observablePropertyIds) {
	...
	private void createProcedures() {
		...
		for (int i = 0; i < procedureIds.length; i++) {
			...
			procedure.setIdentifier(procedureIds[i]);
	...
	private void createValidProcedureTimes() {
		...
		double[] x = new double[] { 7.727958, -117.195711, 6.1320144042060925,
				7.593655600000034, 13.72375999999997, 7.270806,
				4.283393599999954, 10.94306000000006 };
		double[] y = new double[] { 51.883906, 34.056517, 50.78570661296184,
				51.9681661, 51.02881, 51.447722, 52.0464393, 50.68606 };
		...
		String[] shortNames = new String[] { "con terra", "ESRI", "Kisters",
				"IfGI", "TU-Dresden", "Hochschule Bochum", "ITC", "DLZ-IT" };

		for (int i = 0; i < procedureIds.length; i++) {
			ValidProcedureTime validProcedureTime = new ValidProcedureTime();
			validProcedureTime.setProcedure(getProcedure(procedureIds[i]));
			validProcedureTime.setStartTime(startDate);
			validProcedureTime.setDescriptionXml(createSensorDescription(
					procedureIds[i], observablePropertyIds[i], x[i], y[i], 0.0,
					longNames[i], shortNames[i]));
	...
	private void createObservationConstellations() {
		...
		for (int i = 0; i < procedureIds.length; i++) {
			...
			oc.setProcedure(getProcedure(procedureIds[i]));
			oc.setObservableProperty(getObservableProperty(observablePropertyIds[i]));
			oc.setOffering(getOffering(offeringIds[i]));
	...
	private <T extends Observation> T setObservation(T observation,
			String procedure, String observableProperty, String feature,
			String unit, String offering, Date date) {
		observation.setProcedure(getProcedure(procedure));
		observation
				.setObservableProperty(getObservableProperty(observableProperty));
	...
	private void createNumericObservations(int identifierIndex) {
		BigDecimal[] values = new BigDecimal[] { new BigDecimal(1.2),
				new BigDecimal(1.3), new BigDecimal(1.4), new BigDecimal(1.5),
				new BigDecimal(1.6), new BigDecimal(1.7), new BigDecimal(1.8),
				new BigDecimal(1.9), new BigDecimal(2.0), new BigDecimal(2.1) };
		...
		for (int i = 0; i < values.length; i++) {
			...
			setObservation(observation, procedureIds[identifierIndex],
					observablePropertyIds[identifierIndex],
					featureIds[identifierIndex], unitIds[identifierIndex],
					offeringIds[identifierIndex], calendar.getTime());
	...
	private void createCountObservations() {
		...
		for (int i = 0; i < values.length; i++) {
			...
			setObservation(observation, procedureIds[1],
					observablePropertyIds[1], featureIds[1], unitIds[1],
					offeringIds[1], calendar.getTime());
			calendar.add(Calendar.MINUTE, 1);
			observation.setValue(values[i]);
	...
	private void createBooleanObservations() {
		...
		for (int i = 0; i < values.length; i++) {
			...
			setObservation(observation, procedureIds[2],
					observablePropertyIds[2], featureIds[2], unitIds[2],
					offeringIds[2], calendar.getTime());
			calendar.add(Calendar.MINUTE, 1);
			observation.setValue(values[i]);
	...
	private void createCategoryObservations() {
		String[] values = new String[] { "test_category_1", "test_category_2",
				"test_category_1", "test_category_5", "test_category_4",
				"test_category_3", "test_category_1", "test_category_2",
				"test_category_1", "test_category_6" };

		GregorianCalendar calendar = new GregorianCalendar(2012, 11, 19, 13, 00);
		for (int i = 0; i < values.length; i++) {
			CategoryObservation observation = new CategoryObservation();
			setObservation(observation, procedureIds[3],
					observablePropertyIds[3], featureIds[3], unitIds[3],
					offeringIds[3], calendar.getTime());
			calendar.add(Calendar.MINUTE, 1);
			observation.setValue(values[i]);
	...
	private void createTextObservations() {
		String[] values = new String[] { "test_text_0", "test_text_1",
				"test_text_2", "test_text_3", "test_text_4", "test_text_5",
				"test_text_6", "test_text_7", "test_text_8", "test_text_10" };

		GregorianCalendar calendar = new GregorianCalendar(2012, 11, 19, 13, 00);
		for (int i = 0; i < values.length; i++) {
			TextObservation observation = new TextObservation();
			setObservation(observation, procedureIds[0],
					observablePropertyIds[4], featureIds[4], unitIds[4],
					offeringIds[4], calendar.getTime());
			calendar.add(Calendar.MINUTE, 1);
			observation.setValue(values[i]);
	...
	private void createResultTemplates() {
		...
		template.setProcedure(getProcedure(procedureIds[5]));
		template.setObservableProperty(getObservableProperty(observablePropertyIds[5]));
		template.setOffering(getOffering(offeringIds[5]));
		template.setFeatureOfInterest(getFeatureOfInterest(featureIds[5]));
		template.setIdentifier(procedureIds[5] + "/template/1");
	...
	private void createSpecialObservations() {
		NumericObservation o1 = createSpecialObservation(
				"http://www.52north.org/test/observation/1",
				new GregorianCalendar(2012, 11, 19, 13, 10));
		NumericObservation o2 = createSpecialObservation(
				"http://www.52north.org/test/observation/2",
				new GregorianCalendar(2012, 11, 19, 13, 15));

		session.save(o1);
		session.save(o2);
	}

	private NumericObservation createSpecialObservation(String id,
			GregorianCalendar calendar) {
		NumericObservation observation = new NumericObservation();
		observation.setProcedure(getProcedure(procedureIds[0]));
		observation
				.setObservableProperty(getObservableProperty(observablePropertyIds[0]));
		observation.setFeatureOfInterest(getFeatureOfInterest(featureIds[0]));
		observation.setUnit(getUnit(unitIds[0]));
		observation.getOfferings().add(getOffering(offeringIds[0]));
	...
	private static String createSensorDescription(String uniqueId,
			String observableProperty, double x, double y, double z,
			String longName, String shortName) {
		return "<sml:SensorML version=\"1.0.1\" "

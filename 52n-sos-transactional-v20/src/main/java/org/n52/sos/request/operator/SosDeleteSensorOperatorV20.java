...
public class SosDeleteSensorOperatorV20 implements IRequestOperator {
    ...
    public ServiceResponse receiveRequest(AbstractServiceRequest request) throws OwsExceptionReport {
        if (request instanceof DeleteSensorRequest) {
            DeleteSensorRequest sosRequest = (DeleteSensorRequest) request;
            checkRequestedParameter(sosRequest);
            DeleteSensorResponse response = this.dao.deleteSensor(sosRequest);
    ...
    public Set<String> getConformanceClasses() {
        ...
        if (hasImplementedDAO()) {
            conformanceClasses.add("http://www.opengis.net/spec/SOS/2.0/conf/sensorDeletion");
        }
